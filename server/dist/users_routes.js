...
const gq = __importStar(require("./games_queries"));
...
router.post('/login', async (req, res, _next) => {
    ...
    let comparison;
    try {
        comparison = await bcrypt_1.default.compare(password, hashedPass);
    }
    catch (error) {
        res.status(400).send();
        return;
...
router.get('/account', async (req, res, _next) => {
...
router.get('/envelopeinfo', async (req, res, _next) => {
    if (!req.session.userId) {
...
router.get('/transactioninfo', async (req, res, _next) => {
    if (!req.session.userId) {
...
router.post('/password', async (req, res, _next) => {
    if (!req.session.userId) {
    ...
    let hashedNewPass;
    try {
        hashedNewPass = await bcrypt_1.default.hash(newPassword, 10);
    }
    catch (error) {
        res.status(400).send();
        return;
...
router.post('/addenvelope', async (req, res, _next) => {
    const userId = req.session.userId;
    if (!userId) {
...
router.post('/removeenvelope', async (req, res, _next) => {
    const userId = req.session.userId;
    if (!userId) {
...
router.post('/updateenvelope', async (req, res, _next) => {
    const userId = req.session.userId;
    if (!userId) {
...
router.post('/addtransaction', async (req, res, _next) => {
    const userId = req.session.userId;
    if (!userId) {
